...
(function (): void {
    ...
    /** A = 视觅隐藏 + /hide 命令； B = 仅 /hide 命令 */
    type HideMode = 'A' | 'B';
    let currentMode: HideMode = 'A';
     ...
     */
    function callSlashCommand(command: string): void {
     ...
     * @param end   - 结束楼层（含）
     */
    function hideFloors(start: number, end: number): void {
     ...
     * @param end   - 结束楼层（含）
     */
    function unhideFloors(start: number, end: number): void {
        if (!isValidRange(start, end)) return;
        if (start > end) [start, end] = [end, start];
    ...
    /** 将楼层范围转换为可读的中文标签 */
    function buildRangeLabel(start: number, end: number): string {
    ...
    /** 从 UI 输入框读取起始/结束楼层号 */
    function readFloorInputs(): { start: number; end: number } {
        const startEl = document.getElementById('fs-start-floor') as HTMLInputElement | null;
        const endEl   = document.getElementById('fs-end-floor')   as HTMLInputElement | null;
        return {
    ...
    function initModeSelector(): void {
        // 读取上次保存的模式，默认 A
        const saved = localStorage.getItem(MODE_STORAGE_KEY) as HideMode | null;
        currentMode = (saved === 'A' || saved === 'B') ? saved : 'A';

        // 设置 Radio 选中状态
        const radio = document.querySelector<HTMLInputElement>(
            `input[name="fs-hide-mode"][value="${currentMode}"]`
        );
        ...
        // 监听模式切换
        document.querySelectorAll<HTMLInputElement>('input[name="fs-hide-mode"]').forEach(r => {
    ...
    /** 用户切换模式时的回调 */
    function onModeChange(this: HTMLInputElement): void {
        const newMode = this.value as HideMode;
